...
fn findSmallDirsPart2(current_directory: *Directory, required_delete_size: i32) i32 {
    ...
    for (current_directory.children.items) |child| {
        var current_dir_size = findSmallDirsPart2(child, required_delete_size);
    ...
    for (current_directory.children.items) |child| {
        var current_dir_size = getDirSize(child);
        if (current_dir_size >= required_delete_size) {
            dir_size = @min(dir_size, current_dir_size);
        }
    }
...
pub fn main() !void {
    ...
    while (lines.next()) |line| {
        if (line[0] == '$') {
            ...
            var command = command_split.next().?;
            if (mem.eql(u8, command, "cd")) {
                var destination = command_split.next().?;
                if (mem.eql(u8, destination, "..")) {
                    current_directory = current_directory.parent.?;
                } else {
                    l = List(*Directory).init(allocator);
                    f = List(File).init(allocator);
                    var child_directory = try allocator.create(Directory);
            ...
            var size = split.next().?;
            var name = split.next().?;
            var file: File = .{ .name = name, .size = try std.fmt.parseInt(i32, size, 10) };
    ...
    var size = findSmallDirs(current_directory);

    if (ENABLE_DEBUG) {
        debug.print("\n", .{});
    }

    var total_used_size = getDirSize(current_directory);
    var current_unused_space = total_disk_space - total_used_size;
    var required_delete_size = required_unused_space - current_unused_space;
    var part2size = findSmallDirsPart2(current_directory, required_delete_size);
    debug.print("P1: {}, P2: {}\n", .{ size, part2size });
}
