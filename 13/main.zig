...
fn compare(allocator: std.mem.Allocator, leftWithBrackets: []const u8, rightWithBrackets: []const u8, level: i32) !?bool {
    ...
    while (left.len != 0 and right.len != 0) {
        var l = packetSplit(&left);
        var r = packetSplit(&right);

        pprint("- Compare {s} vs {s} \n", .{ l, r }, level);

        if (isList(l) and isList(r)) {
            var comparison = try compare(allocator, l, r, level + 1);
            if (comparison) |c| {
                return c;
            }
        } else if (isList(l) or isList(r)) {
            var lNext = if (!isList(l)) try convertToList(allocator, l) else l;
            var rNext = if (!isList(r)) try convertToList(allocator, r) else r;
            if (isList(l)) {
            ...
            pprint("- Compare {s} vs {s} \n", .{ lNext, rNext }, level);
            var comparison = try compare(allocator, lNext, rNext, level + 1);
            if (comparison) |c| {
                return c;
            }
        } else {
            var lInt = try std.fmt.parseInt(i32, l, 10);
            var rInt = try std.fmt.parseInt(i32, r, 10);
...
fn solve1(allocator: std.mem.Allocator, data: []const u8) !i32 {
    ...
    while (lines.next()) |line| : (pairCounter += 1) {
        ...
        var left = packet.next().?;
        var right = packet.next().?;
        if (ENABLE_DEBUG) {
            print("== Pair {} ==\n", .{pairCounter});
            print("- Compare {s} vs {s}\n", .{ left, right });
        }
        var comparison = try compare(allocator, left, right, 1);
        if (comparison.?) {
            total += pairCounter;
        }
        if (ENABLE_DEBUG) {
...
fn solve2(allocator: std.mem.Allocator, data: []const u8) !i32 {
    ...
    while (lines.next()) |line| : (pairCounter += 1) {
        ...
        var left = packet.next().?;
        var right = packet.next().?;
        var comparison2_1 = try compare(allocator, left, "[[2]]", 1);
        var comparison2_2 = try compare(allocator, right, "[[2]]", 1);
        var comparison6_1 = try compare(allocator, left, "[[6]]", 1);
        var comparison6_2 = try compare(allocator, right, "[[6]]", 1);
        if (comparison2_1.?) {
            two += 1;
        }
        if (comparison2_2.?) {
