...
fn parseMonkeyData(monkeys: *List(Monkey), data: []const u8, allocator: std.mem.Allocator) !void {
    ...
    while (monkeysData.next()) |monkeyData| {
        ...
        while (startingItemsData.next()) |startingItemData| {
            var startingItem = std.mem.trim(u8, startingItemData, " ");
            var item = try std.fmt.parseInt(int, startingItem, 10);
        ...
        var operationString = operationData.next().?;
        var operation = if (std.mem.eql(u8, operationString, "*")) Operation.mul else Operation.add;
        var element2String = operationData.next().?;
        ...
        var divisor = try std.fmt.parseInt(int, divisorslice, 10);
        ...
        var trueMonkey = try std.fmt.parseInt(int, throwSlice, 10);

        throwData = tokenize(u8, behaviours.next().?, " ");
        while (throwData.next()) |d| {
            throwSlice = d;
        }
        var falseMonkey = try std.fmt.parseInt(int, throwSlice, 10);

        var monkey: Monkey = .{ .items = items, .operation = operation, .element2 = element2, .divisor = divisor, .trueMonkey = trueMonkey, .falseMonkey = falseMonkey };
...
fn solve(comptime part2: bool, allocator: std.mem.Allocator) !int {
    const worryDivisor = if (part2) 1 else 3;
    const numberOfRounds = if (part2) 10000 else 20;
    ...
    while (round < numberOfRounds) : (round += 1) {
        ...
        while (monkeyCounter < monkeys.items.len) : (monkeyCounter += 1) {
            while (monkeys.items[monkeyCounter].items.items.len != 0) {
                var item = monkeys.items[monkeyCounter].items.orderedRemove(0);
                var newItem = try monkeyInspect(item, monkeys.items[monkeyCounter], worryDivisor, largeDivisor);
                monkeys.items[monkeyCounter].inspected += 1;
                if (@rem(newItem, monkeys.items[monkeyCounter].divisor) == 0) {
                    try monkeys.items[@intCast(usize, monkeys.items[monkeyCounter].trueMonkey)].items.append(newItem);
                } else {
                    try monkeys.items[@intCast(usize, monkeys.items[monkeyCounter].falseMonkey)].items.append(newItem);
                }
            }
        }
    }
    ...
    std.sort.sort(int, inspections.items, {}, comptime std.sort.desc(int));
