...
fn getShortestDistance(start: Point, end: Point, points: std.AutoHashMap(Point, u8), allocator: std.mem.Allocator) !?int {
    ...
    while (queue.items.len != 0) {
        ...
        inline for (.{ .{ -1, 0 }, .{ 1, 0 }, .{ 0, -1 }, .{ 0, 1 } }) |i| {
            var neigborRow = current.row + i[0];
            var neigborColumn = current.column + i[1];
            var neighbor = Point{ .row = neigborRow, .column = neigborColumn };
            if (points.contains(neighbor) and !distance.contains(neighbor)) {
                var height = points.get(current).?;
                var neighborHeight = points.get(neighbor).?;
...
fn solve(allocator: std.mem.Allocator, data: []const u8, part1: bool) !int {
    ...
    while (lines.next()) |line| : (row += 1) {
        var column: int = 0;
        for (line) |height| {
            var p: Point = .{ .row = row, .column = column };
            switch (height) {
                'S' => {
                    try points.put(p, 'a');
    ...
    for (startPoints.items) |start| {
        var distance = try getShortestDistance(start, end, points, allocator);
        if (distance != null) {
