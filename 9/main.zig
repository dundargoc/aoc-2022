...
fn GetDistanceSquare(a: Point, b: Point) i64 {
    var xSquare = std.math.pow(i64, a.x - b.x, 2);
    var ySquare = std.math.pow(i64, a.y - b.y, 2);
    ...
    while (lines.next()) |i| {
        ...
        while (step < steps) : (step += 1) {
            ...
            while (ropeSegment < ropeLength) : (ropeSegment += 1) {
                ...
                if (distanceSquare >= 4) {
                    var deltaX = std.math.clamp(rope.items[ropeSegment - 1].x - rope.items[ropeSegment].x, -1, 1);
                    var deltaY = std.math.clamp(rope.items[ropeSegment - 1].y - rope.items[ropeSegment].y, -1, 1);
