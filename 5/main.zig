...
fn getStartingStacks(input: []const u8, allocator: Allocator) !Matrix {
    ...
    while (counter < numberOfStacks) : (counter += 1) {
        var innerList = ArrayList(u8).init(allocator);
        try allElements2.append(innerList);
    }

    for (allElements.items) |element, i| {
        if (element != 32) {
...
fn move1(stacks: *Matrix, numberOfMoves: i64, fromIndex: usize, toIndex: usize) !void {
    ...
    while (counter < numberOfMoves) : (counter += 1) {
        var poppedItem = stacks.items[fromIndex].pop();
...
pub fn main() !void {
    ...
    while (instructions.next()) |instruction| {
        ...
        _ = split.next();
        var numberOfMoves = try parseInt(u8, split.next().?, 10);
        _ = split.next();
        var fromIndex = try parseInt(u8, split.next().?, 10) - 1;
        _ = split.next();
        var toIndex = try parseInt(u8, split.next().?, 10) - 1;
